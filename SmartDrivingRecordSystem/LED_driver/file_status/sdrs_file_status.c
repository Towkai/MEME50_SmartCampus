...
#define SUCCESS 0
#define DEVICE_NAME "sdrs_file" // 設備名稱
int Major; // 主設備號
...
static uint32_t pin = 27;  // 默認 GPIO 引腳
...
int __init init_module(void) {
    ...
    charmodule_class = class_create(THIS_MODULE, "file_status_class");
#else
    charmodule_class = class_create("file_status_class"); 
#endif
    ...
    int i = 0;
    device_create(charmodule_class, NULL, MKDEV(Major, i), NULL, "sdrs_file%d", i);
    i++;
#elif LINUX_VERSION_CODE > KERNEL_VERSION(2,6,9)
    device_create(charmodule_class, NULL, MKDEV(Major, 0), "sdrs_file"); 
#else
    class_simple_device_add(charmodule_class, MKDEV(Major, 0), NULL, "sdrs_file");
#endif
    printk(KERN_ALERT "'mknod /dev/sdrs_file c %d 0'.\n", Major);
